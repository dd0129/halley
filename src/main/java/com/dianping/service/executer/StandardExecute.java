...
public class StandardExecute {
    ...
    private Integer getRunningJobNum() throws Exception {
        String sql = "select count(1) as num from etl_task_status where status =? and type =2 and database_src=?";
        return Long.valueOf(this.jdbcTemplate.queryForLong(sql, new Object[]{SystemConstant.JOB_RUNNING, this.database_src})).intValue();
    }

    public void execute() {
        TaskStatus ts = null;
        try {
            logger.info("the standard " + this.database_src + " execute process starts");
            int num = this.getRunningJobNum();
            logger.info("the standard " + this.database_src + " running job num is " + num);
            if (num >= this.run_num) {
                return;
            }
            ...
            logger.info("the standard " + this.database_src + " execute process ends");
        }
    }
    ...
    private TaskStatus getReadyTask() throws Exception {
        String sql1 = "select task_status_id,task_name,trigger_time,task_obj,para1,para2,para3,log_path,task_id,cycle,time_id,type,if_pre,if_wait,success_code,wait_code from etl_task_status a " +
                "where status =? and type=2 and database_src=? order by running_prio desc,prio_lvl,time_id,run_num,rand() ";
        ...
        for (Map<String, Object> map : list) {
            ...
            if (ts != null && this.getSameJobRunCnt(ts) > 0) {
                logger.info(ts.getTime_id() + "(" + ts.getTask_name() + ") has running job");
                continue;
            } else {
                if (runningQueue.containsKey(ts.getTask_status_id())) {
